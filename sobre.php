<!DOCTYPE html>
<html lang="pt-br">

  <head>
    <meta charset="utf-8">

    <title> E-Solar - Projeto de TCC</title>

    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="Sistema para Automacao e controle de recursos hidrícos de uma residencia">
    <meta name="author" content="Felipe Alves, Gabriel Lins, Guilherme Teixeira">

    <!-- Links do estilo -->
    <link rel="stylesheet" type="text/css" href="css/bootstrap.min.css" >
    <link rel="stylesheet" type="text/css" href="css/bootstrap.css">
    <link rel="stylesheet" type="text/css" href="css/style.css">

    <script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></script>
    
   
<style>
    
    body {

       color: rgba(0, 0, 0, 0.9);
    }
    

    p {

        text-align:left;
        text-indent:1px; 
    }

</style>
    <!-- HTML5 shim, for IE6-8 support of HTML5 elements -->
    <!--[if lt IE 9]>
      <script src="../assets/js/html5shiv.js"></script>
    <![endif]-->


    <!-- Favicon Icone Projeto -->
    <link rel="shortcut icon" href="img/favico.png">

  </head>

  <body>
   
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
      <div class="container">
        <div class="navbar-header">
          <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar">
            <span class="sr-only">Toggle navigation</span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
          </button>
          <a class="navbar-brand" href="index.php"><img src="img/logo-geragua.gif" height="80"></a>
        </div>
        <div id="navbar" class="navbar-collapse collapse">
          <ul id="insidepagenav" class="nav navbar-nav navbar-right">
                      <li><a href="index.php"><span class="glyphicon glyphicon-home"></span> Inicio</a></li>
                      <li><a href="#noticias"><span class="glyphicon glyphicon-tasks"></span> Notícias</a></li>
                      <li><a href="projeto.php"><span class="glyphicon glyphicon-paperclip"></span> Projeto</a></li>
                      <li><a href="fotos.php"><span class="glyphicon glyphicon-camera"></span> Fotos</a></li>     
                      <li><a href="sobre.php"><span class="glyphicon glyphicon-phone-alt"></span> Quem Somos</a></li>
                  </ul>
        </div><!--/.navbar-collapse -->
      </div>
    </nav>



    <div class="container conteudo">

      	<div class="col-xs-12">
      		<h4 class="cor">Equipe</h4>
      		<div class="row">
      			<div class="col-xs-4 bfotos ">
              <div id="interface">
                <!--Grafico aqui-->

                  <figure class="foto-legenda">
                    <img src="img/felipe.jpg">
                    <figcaption>
                      <h2>Felipe Alves</h2>
                    </figcaption
                  </figure>
                   
                  
            

              </div>
      			</div>

            <div class="col-xs-4 bfotos ">
              <div id="interface">
                <!--Grafico aqui-->

                  <figure class="foto-legenda">
                    <img src="img/gabriel.jpg">
                    <figcaption>
                      <h2>Gabriel</h2>
                    </figcaption
                  </figure>
                   
                  
            

              </div>

            </div><div class="col-xs-4 bfotos ">
              <div id="interface">
                <!--Grafico aqui-->

                  <figure class="foto-legenda">
                    <img src="img/guilherme.jpg">
                    <figcaption>
                      <h2>Guilherme</h2>
                    </figcaption
                  </figure>
                   
                  
            

              </div>
            </div>
      		</div>

         
          
      </div>
      <hr>
      <footer>
      <div class="row">
      	<div class="col-xs-12 footer">
      	  <h4> Copyright &copy; - Apresentação - Feira Regional 2014</h4>
      	</div>
      </div>
      </footer>
    </div> 
    <!-- /container -->
    <script src="js/bootstrap.js" type="text/javascript"></script>
    <script src="js/bootstrap.min.js" type="text/javascript"></script>
    <script src="http://code.highcharts.com/highcharts.js"></script>
    <script src="http://code.highcharts.com/modules/exporting.js"></script>
  </body>
</html>